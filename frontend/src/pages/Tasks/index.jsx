...
const Task = () => {
  ...
  const completeTask = async (tasks_id) => {
    if(id) {
      await axios.put(`http://localhost:5000/api/tasks/${tasks_id}`, {
        status: "completed"
      })
      .then((res) => {
  ...
  const deleteTask = async (tasks_id) => {
    if(id) {
      await axios.delete(`http://localhost:5000/api/tasks/${tasks_id}`, {
        data: {id: id},
      })
      .then((res) => {
  ...
  useEffect(() => {
    if(id) {
      const fetch = async () => {
        await axios.get(`http://localhost:5000/api/tasks/${id}`)
        .then((res) => {
